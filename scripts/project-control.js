...
function destination(args) {
  ...
  if (args.create) {
    ...
    fs.writeFileSync(path.join(projectDestination, '.harness', 'state', 'active-task.json'), `${JSON.stringify({
      ...
      id: 'project-intake',
      goal: 'Model the system, establish an architecture hypothesis, and define the first complete connected slice.',
      status: 'blocked',
