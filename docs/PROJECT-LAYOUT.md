...
├── project-harness/                reusable harness source
└── projects/                       configured workspace root
    └── super-clean-deals/          canonical project and Git repository
```

After destination creation, open `super-clean-deals/` as the VS Code workspace. The agent resumes, plans, codes, tests, and checkpoints from that project root. It must not create product code inside `project-harness/`.
